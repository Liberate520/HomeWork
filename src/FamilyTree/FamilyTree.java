...
public class FamilyTree {
    ...
    public FamileTree() {this(new ArrayList<>()); }

    public boolean add(Human human) {
        if (human == null) {
            return false;
        }
        if (!humanList.contain(human)){
            humanList.add(human);
...
private void addToChildren(Human human){
    ...
    public List<Human> getSiblings(int id){
        ...
        for (Human parent: human.getParents()){
            for (Human cgild: parent.getChildren()){
...
public bollean setDivorce(long humanId1, long humanId2) {
    if (checkId1) && checkId(humanId2)){
    Human human1 = getById(humanId1);
